...
function CashierDashboardRoute() {
  ...
  interface NotificationItem {
    id: string;
    entityId: string; // ID único del pedido para evitar duplicaciones
    type: "order";
    title: string;
    subtitle: string;
    detail: string;
    ...
    <div className="doc-legible min-h-screen bg-[#F9F8F3] text-[#231A14] pb-20 font-sans selection:bg-[#D4AF37] selection:text-[#2D473C]">
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col gap-3 max-w-sm sm:max-w-md w-full pointer-events-none">
        {notifications.map((notif) => (
          <div
            ...
            className="pointer-events-auto p-4 rounded-2xl shadow-2xl border-2 flex items-center gap-3.5 animate-in slide-in-from-top-4 duration-300 cursor-pointer transition-all hover:scale-102 bg-[#1E2E27] text-white border-[#F97316]"
          >
            {/* Ícono distintivo */}
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 font-bold bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/30 animate-bounce">
              <ShoppingBag size={22} className="text-white" />
            </div>

            {/* Texto de la comanda */}
            <div className="flex-1 min-w-0">
              <span className="text-xs font-sans font-black uppercase tracking-wider block text-orange-400">
                {notif.title}
              </span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-2 rounded-2xl border border-gray-200 shadow-2xs">
          ...
          {/* Main Module Bar (Comandas / Arqueo / Stock / Yape) */}
          <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            <span className="flex-1 sm:flex-none py-2.5 px-5 rounded-xl text-xs font-sans transition-all flex items-center justify-center gap-2 bg-[#2D473C] text-white shadow-md font-black border border-[#2D473C]">
